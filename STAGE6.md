...
- [ ] Self-метрики Р41 через провайдеры Р27; сверка с `/metrics` и OTLP.
- [ ] `tests/e2e/`: вариант compose с `postgres -c ssl=on` (+ серверный
      серт/ключ), pgbench/psql `sslmode=require`; ассерт — те же ряды
      `latkit_query_duration_seconds`/`latkit_queries_total`, что в
      plaintext-прогоне, плюс `latkit_tls_connections>0`. (Стенд:
      захват по IP контейнера; TLS-инстанс — `pg-ssl-test:5434`.)
- [ ] `docs/notes-tls.md`: схема захвата (uprobe entry/ret, направления),
...
- [ ] README: флаги `--tls`/`--libssl`/`--tls-comm` + env, требование
      `hostPID`/CAP для uprobes; **security-замечание PLAN.md §7
      дополнить**: по TLS-соединениям агент так же видит текст SQL
      (маскирование литералов — тот же дефолт, спаны с сырым SQL — так же
      за флагом, Р32).
- [ ] PLAN.md §4: отметить этап 6 выполненным; §5 «TLS повсеместен» —
      митигация закрыта; зафиксировать known gaps (GSSENC, unix-сокеты
      остаются v1.1).
...
- [ ] `psql`/pgbench с `sslmode=require` под нагрузкой ≥ 30 мин →
      корректные latency-гистограммы и нормализованные запросы,
      **совпадающие** с plaintext-прогоном той же нагрузки (в пределах
      джиттера); лейблы `db`/`user` присутствуют (из расшифрованного
      `StartupMessage`).
- [ ] На TLS-соединении ciphertext не попадает во framer/парсер (grep по
      трафику стенда: во внутренних буферах — только плейнтекст протокола,
      никаких TLS-рекордов); `corr_miss` ≈ 0 на штатных сессиях.
- [ ] `--tls auto` находит libssl постгреса на хосте и в контейнере;
      отсутствие libssl → мягкая деградация (метрика `state="none"`,
      агент жив на plaintext); битый `--libssl` → фатально на старте.
- [ ] Смешанная нагрузка (часть соединений TLS, часть plaintext
      одновременно) → оба типа считаются корректно, seq-пространства не
      мешают друг другу, потери одного канала не марают другой.
- [ ] Fork backend'ов postgres покрыт (attach `pid=-1`); рестарт postgres
      с новым путём libssl — ре-скан доаттачивает без перезапуска агента.
- [ ] Unit (`test_tls_route`, сброс framer'а) и replay (`ssl_tls.lkt` →
      полная сессия) зелёные без привилегий/BPF; ASAN/UBSAN чисты.
- [ ] `src/proto/`, `src/norm/`, `src/metrics/`, `src/export/` не изменены
      (границы контрактов Р15 держат); новый libbpf-код изолирован в
      `tls_attach.c` и `latkit.bpf.c`.
- [ ] Документация (`notes-tls.md`, README security-замечание, env-таблица)
