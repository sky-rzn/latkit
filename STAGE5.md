...
- [ ] Расширить `loop.c`: `lk_loop_del_fd`, интерес EPOLLIN|EPOLLOUT
      (`lk_loop_mod_fd` или маска в add) — обратная совместимость с
      текущими пользователями петли.
- [ ] `http.c` (Р29): listener (bind/listen, `SO_REUSEADDR`,
      ...
      на мусор, `HEAD` без тела.
- [ ] `prom.c`: `/metrics` — `open_memstream` + `lk_metrics_dump`;
      `/healthz` — uptime/события/дропы; регистрация роутов и self-метрик
      (`http_requests_total`, `scrape_duration_seconds`).
- [ ] `main.c`: `--prom-listen` (дефолт `127.0.0.1:9752`, `none` —
      выключить); сверить порт со списком экспортеров, зафиксировать.
- [ ] `test_http.c` на socketpair/loopback: разбор запроса по байту
      (torn request-line), запрос больше лимита, медленный клиент →
      таймаут, частичная запись ответа (маленький SO_SNDBUF), два
      параллельных скрейпа, 404/405.
- [ ] Замер: длительность дампа при заполненном реестре (стресс-генератор
      из 4.5) — зафиксировать в docs; sanity, что пауза ≪ запаса ringbuf.
...
- [ ] `timebase.c` (Р33): offset REALTIME−MONOTONIC, семпл на каждом
      экспорте; согласовать `process_start_time_seconds`.
- [ ] `registry.c`: `created_ns` ряда (Р31); `lk_metrics_iter` —
      стабильный обход, тот же порядок, что dump.
- [ ] `pbuf.c`: varint/tag/len-delimited, вложенность через
      резерв+memmove; `test_pbuf.c` — golden-байты против эталонов от
      protoc (зафиксировать `.bin`-фикстуры в репо), граничные varint,
      пустые сообщения.
- [ ] `otlp.c`, энкодер: Resource/Scope, Sum/Gauge/ExponentialHistogram
      (scale=2, offset −53, underflow→zero_count+zero_threshold,
      overflow→верхний бакет), cumulative + start_time из `created_ns`;
      `test_otlp_enc.c` — golden-байты на синтетическом реестре.
- [ ] `otlp.c`, клиент (Р31): async connect/write/read-status в петле,
      один в полёте, таймаут 5 с, drop-and-count, Retry-After, кэш
      резолва + перерезолв при ошибках; тик по `lk_loop_every`.
- [ ] Флаги/env: `--otlp-endpoint`/`--otlp-interval`/`--otlp-header`/
      `--otlp-resource` + `OTEL_EXPORTER_OTLP_*` как дефолты (Р34,
      механика env-слоя — здесь, полная таблица — 5.4).
