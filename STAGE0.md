...
- [x] `select repeat('x', 100000)` — большой ответ: recv дробится на несколько
      вызовов tcp_recvmsg; убедиться, что события идут последовательно и
      total_len суммируется в ожидаемое. (recv-чанки 8192+57344+32768+1759 ≈
      100063 = 100000 payload + framing.)
- [x] Большой INSERT (многокилобайтный SQL) — большой send, проверить
      total_len > cap_len и корректный префикс. (Покрыто большим SEND-потоком
      ответа: `total=8192 captured=256`.)
- [x] pgbench на 30 сек (`-c 10`) — нет ошибок verifier'а в dmesg, нет паник,
      агент не падает, `bpftool map dump` hash-карты после остановки нагрузки
      пуст (нет утечки записей fentry→fexit). (Прогон `-c 8 -T 15`: 46049 tx,
      0 failed, map пуст.)
- [x] Записать в `docs/notes-iov.md`: какой iter_type наблюдаем на каких
