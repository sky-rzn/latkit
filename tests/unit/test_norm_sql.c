...
/* Table-driven tests for the SQL normaliser (task 4.1, Р22). Drives the public
 * lk_norm_sql over hand-written cases and checks:
 *
...
static int test_garbage(void)
{
    ...
    /* non-UTF-8 / control-byte garbage: must not crash and must be stable */
    {
        ...
        lk_norm_sql(junk, sizeof(junk), &g1);
        lk_norm_sql(junk, sizeof(junk), &g2);
        CHECK(g1.fp == g2.fp);
